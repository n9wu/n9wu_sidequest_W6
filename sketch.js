...
let playerAnis = {
  idle: { row: 0, frames: 9, frameDelay: 10 },
  run: { row: 1, frames: 8, frameDelay: 3 },
  jump: { row: 2, frames: 4, frameDelay: Infinity, frame: 0 },
...
// flag used to defer heavy work (level rebuild) until draw() begins
let levelChangeScheduled = false;
...
/*
  Tile key:
    ...
    f = fire hazard (player takes damage, boars turn around if they "see" it ahead)
      = empty (no sprite)
*/
// first level (simplified for easier play – fewer boars/fires, still 15 mushrooms)
let level1 = [
  "                    g   g     x        ", // row  0  (no boar, one leaf)
  "                x                     ", // row  1  (leaf instead of boar)
  "      x         LgR         LggR         ", // row  2  (fire removed)
  "     LR   LgR          LR x             ", // row  3  (single leaf at end)
  "   x     b        x   b                ", // row  4  (two leaves, two boars)
  "   LgggR   x   LR   LgR x              ", // row  5  (two leaves, no boar)
  "         LgR    x       g   LggggR      ", // row  6  (one leaf, no boar)
  "               LgR                    x   ", // row  7  (one leaf far right)
  " LgR   x                           LggR", // row  8  (one leaf, no boar)
  "         LgR        f x    LR  LgR  [dd]", // row  9  (one fire, one leaf)
  "   x     [d]      x LggR   x    f   [dd]", // row 10 (three leaves, single fire)
  "LgggR   f   gggggggR    Lgggg]    ggggggg", // row 11 (no hazards)
...
// we'll recalc values whenever the active level changes
function updateLevelMetrics() {
  LEVELW = TILE_W * level[0].length;
  LEVELH = TILE_H * level.length;
  PLAYER_START_Y = LEVELH - TILE_H * 4;

  // count leaves ('x') on the map to know how many to collect
  WIN_SCORE = level.reduce(
    (sum, row) => sum + (row.match(/x/g) || []).length,
    0,
  );
}
...
let WIN_SCORE; // will be recalculated based on current level

let PLAYER_START_Y; // set by updateLevelMetrics()

// player damage knockback tuning
const PLAYER_KNOCKBACK_X = 2.0;
const PLAYER_KNOCKBACK_Y = 3.2;
// raised jump so the player can clear fire hazards more easily
const PLAYER_JUMP = 5.5;
...
function handleLevelChange() {
  ...
  if (platformsL) platformsL.removeAll();
  if (platformsR) platformsR.removeAll();
  if (wallsL) wallsL.removeAll();
  if (wallsR) wallsR.removeAll();
  if (leaf) leaf.removeAll();
  ...
  // refresh spawn arrays exactly like setup
  ...
  // immediately center camera on player (avoids mis-constraints)
  camera.x = player.x;
  camera.y = player.y;
}
...
function preload() {
  ...
  sndJump = loadSound("assets/jump.wav");
  sndMusic = loadSound("assets/background music.wav");
  sndCollect = loadSound("assets/collect.wav");
}
...
function setup() {
  ...
  // background music should start and loop
  if (sndMusic) {
...
function rescueLeaf(player, leaf) {
  ...
  // win condition
  if (score >= WIN_SCORE) {
    // if there is another level available, queue a transition
...
// --- PLAYER ATTACK -> BOAR ---
function tryHitBoar() {
  // reuse the same grounded logic used for movement so that being on
  // deep ground tiles doesn't disable the hit detection.
  const grounded = isPlayerGrounded();
  if (!grounded) return;
...
function updateBoars() {
  ...
  for (const e of boar) {
    ...
    // 4) turn if another boar is directly ahead – prevents bunching/stuck packs
    let frontHitsBoar = false;
    for (const other of boar) {
      if (other === e || other.dead || other.dying) continue;
      if (e.frontProbe.overlapping(other)) {
        frontHitsBoar = true;
        break;
      }
    }
